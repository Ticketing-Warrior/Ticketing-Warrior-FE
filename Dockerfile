...
# # 5️⃣ 빌드 실행
# RUN npm run build   
# # Vite가 /dist (또는 /build) 폴더에 정적 파일을 만든다
...
# 1️⃣ 베이스 이미지 (대문자 FROM)
# 파일 추출을 위해 이 스테이지에 'artifact'라는 이름(AS artifact)을 명확하게 지정합니다.
...
# ------------------------------------------------------------
# Dockerfile의 나머지 부분(FROM nginx:stable-alpine AS runtime 이하)은 모두 삭제합니다.
# 이 파일은 이제 Nginx 이미지를 만들지 않고, 
# 오직 'artifact' 스테이지에서 빌드 결과물을 추출하는 용도로만 사용됩니다.